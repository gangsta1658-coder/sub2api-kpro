...
// GetDailyCheckIn GET /user/check-in
func (h *UserHandler) GetDailyCheckIn(c *gin.Context) {
	...
	result, err := h.userService.GetDailyCheckIn(c.Request.Context(), subject.UserID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
// GetDailyCheckInCalendar GET /user/check-in/calendar
func (h *UserHandler) GetDailyCheckInCalendar(c *gin.Context) {
	...
	result, err := h.userService.GetDailyCheckInCalendar(c.Request.Context(), subject.UserID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
// ClaimDailyCheckIn POST /user/check-in
