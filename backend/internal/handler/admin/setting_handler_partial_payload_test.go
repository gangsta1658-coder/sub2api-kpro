...
func TestUpdateSettingsIPBlacklistOmittedPreservesStoredRules(t *testing.T) {
	h, repo := newStepUpSwitchTestHandler(t, map[string]string{
		service.SettingKeyIPBlacklist: `[
  "47.92.239.70",
  "203.0.113.0/24"
]`,
	})
  ...
  "47.92.239.70",
  "203.0.113.0/24"
]`, repo.values[service.SettingKeyIPBlacklist])
}

func TestUpdateSettingsIPBlacklistExplicitEmptyArrayClearsRules(t *testing.T) {
	h, repo := newStepUpSwitchTestHandler(t, map[string]string{
		service.SettingKeyIPBlacklist: `["47.92.239.70"]`,
	})
	...
	require.Equal(t, http.StatusOK, rec.Code)
	require.JSONEq(t, `[]`, repo.values[service.SettingKeyIPBlacklist])
}
