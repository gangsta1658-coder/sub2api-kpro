...
func (h *SettingHandler) UpdateSettings(c *gin.Context) {
	...
	// ip_blacklist is managed by its own CRUD endpoints as well as this
	// whole-document settings endpoint. Preserve the value currently in the
	// database when the field was omitted so a stale settings snapshot cannot
	// overwrite a concurrent blacklist change. An explicitly supplied empty
	// array remains a deliberate clear (the pointer is non-nil).
	if req.IPBlacklist == nil {
		omitted[service.SettingKeyIPBlacklist] = struct{}{}
	}
