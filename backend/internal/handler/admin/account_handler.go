...
func (h *AccountHandler) List(c *gin.Context) {
	...
	if len(folder) > 64 {
		response.ErrorFrom(c, infraerrors.BadRequest("INVALID_FOLDER_FILTER", "invalid folder filter"))
		return
	}
	if folder != "" {
		requestContext := service.WithAccountFolderFilter(
			c.Request.Context(),
			folder,
			folder == service.AccountFolderUncategorizedFilter,
		)
...
// ListFolders returns persisted account folder names and counts.
// GET /api/v1/admin/accounts/folders
func (h *AccountHandler) ListFolders(c *gin.Context) {
	...
	for page := 1; ; page++ {
		accounts, total, err := h.adminService.ListAccounts(
			c.Request.Context(), page, pageSize, "", "", "", "", 0, "", "name", "asc",
		)
		if err != nil {
			response.ErrorFrom(c, err)
			return
		...
		for _, account := range accounts {
			folder, _ := account.Extra[service.AccountFolderExtraKey].(string)
	...
	folders := make([]folderSummary, 0, len(folderCounts))
	for name, count := range folderCounts {
		folders = append(folders, folderSummary{Name: name, Count: count})
	}
			...
			if models, fetchErr := h.accountTestService.FetchOpenAIAccountModels(c.Request.Context(), account); fetchErr == nil {
				for i := range models {
					if strings.TrimSpace(models[i].DisplayName) == "" {
						models[i].DisplayName = models[i].ID
					}
				}
