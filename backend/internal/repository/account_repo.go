...
func (r *accountRepository) accountListFilteredQuery(ctx context.Context, platform, accountType, status, search string, groupID int64, privacyMode string) *dbent.AccountQuery {
	...
	if folderFilter, ok := service.AccountFolderFilterFromContext(ctx); ok {
		q = q.Where(dbpredicate.Account(func(s *entsql.Selector) {
			path := sqljson.Path(service.AccountFolderExtraKey)
			...
			s.Where(sqljson.ValueEQ(dbaccount.FieldExtra, folderFilter.Name, path))
		}))
	}
