...
var _ service.DailyCheckInRepository = (*userRepository)(nil)
...
func (r *userRepository) GetDailyCheckIn(ctx context.Context, userID int64, checkInDate string) (service.DailyCheckIn, error) {
	const query = `
		SELECT
			FALSE,
			COALESCE((SELECT reward FROM daily_checkins WHERE user_id = u.id AND checkin_date = $2), 0),
			u.balance,
			(SELECT created_at FROM daily_checkins WHERE user_id = u.id AND checkin_date = $2)
		FROM users AS u
		WHERE u.id = $1 AND u.deleted_at IS NULL
	`
	return r.scanDailyCheckIn(ctx, checkInDate, query, userID, checkInDate)
}

func (r *userRepository) ListDailyCheckInDates(ctx context.Context, userID int64, fromDate, toDate string) ([]string, error) {
	const query = `
		SELECT d.checkin_date::text
		FROM daily_checkins AS d
		JOIN users AS u ON u.id = d.user_id AND u.deleted_at IS NULL
		WHERE d.user_id = $1
		  AND d.checkin_date >= $2::date
		  AND d.checkin_date < $3::date
		ORDER BY d.checkin_date ASC
	`

	rows, err := clientFromContext(ctx, r.client).QueryContext(ctx, query, userID, fromDate, toDate)
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
...
func (r *userRepository) ClaimDailyCheckIn(ctx context.Context, userID int64, checkInDate string, reward float64) (service.DailyCheckIn, error) {
	const query = `
		WITH inserted AS (
			INSERT INTO daily_checkins (user_id, checkin_date, reward)
			SELECT id, $2::date, $3 FROM users WHERE id = $1 AND deleted_at IS NULL
			ON CONFLICT (user_id, checkin_date) DO NOTHING
			RETURNING reward, checkin_date, created_at
		), updated AS (
			UPDATE users AS u
			SET balance = u.balance + i.reward, updated_at = NOW()
			FROM inserted AS i
			WHERE u.id = $1 AND u.deleted_at IS NULL
			RETURNING u.balance
		)
		SELECT
			...
			COALESCE((SELECT reward FROM inserted), (SELECT reward FROM daily_checkins WHERE user_id = $1 AND checkin_date = $2::date)),
			COALESCE((SELECT balance FROM updated), (SELECT balance FROM users WHERE id = $1 AND deleted_at IS NULL)),
			COALESCE((SELECT created_at FROM inserted), (SELECT created_at FROM daily_checkins WHERE user_id = $1 AND checkin_date = $2::date))
	`
	return r.scanDailyCheckIn(ctx, checkInDate, query, userID, checkInDate, reward)
}

func (r *userRepository) scanDailyCheckIn(ctx context.Context, checkInDate, query string, args ...any) (service.DailyCheckIn, error) {
	rows, err := clientFromContext(ctx, r.client).QueryContext(ctx, query, args...)
	if err != nil {
		return service.DailyCheckIn{}, err
	}
	defer func() { _ = rows.Close() }()
	if !rows.Next() {
		if err := rows.Err(); err != nil {
		...
		return service.DailyCheckIn{}, service.ErrUserNotFound
	}

	var result service.DailyCheckIn
	var reward sql.NullFloat64
	var balance sql.NullFloat64
	var checkedInAt sql.NullTime
	if err := rows.Scan(&result.NewlyClaimed, &reward, &balance, &checkedInAt); err != nil {
		return service.DailyCheckIn{}, err
	}
	if err := rows.Err(); err != nil {
		return service.DailyCheckIn{}, err
	...
	result.Reward = reward.Float64
	result.Balance = balance.Float64
	result.ClaimedToday = checkedInAt.Valid
	if checkedInAt.Valid {
		checkedInAtValue := checkedInAt.Time
		result.CheckedInAt = &checkedInAtValue
	}
