...
const DailyCheckInReward = 0.2

// DailyCheckIn records the current user's daily reward status.
...
// DailyCheckInCalendar contains the current server day and this month's
...
// DailyCheckInRepository keeps the daily reward record and balance update atomic.
type DailyCheckInRepository interface {
	GetDailyCheckIn(ctx context.Context, userID int64, checkInDate string) (DailyCheckIn, error)
	ClaimDailyCheckIn(ctx context.Context, userID int64, checkInDate string, reward float64) (DailyCheckIn, error)
}

// DailyCheckInCalendarRepository reads completed check-ins without changing
// the atomic claim path above.
type DailyCheckInCalendarRepository interface {
	ListDailyCheckInDates(ctx context.Context, userID int64, fromDate, toDate string) ([]string, error)
}
...
type UserService struct {
	...
	dailyCheckInRepo         DailyCheckInRepository
	dailyCheckInCalendarRepo DailyCheckInCalendarRepository
...
func NewUserService(userRepo UserRepository, settingRepo SettingRepository, authCacheInvalidator APIKeyAuthCacheInvalidator, billingCache BillingCache) *UserService {
	return &UserService{
		userRepo:                 userRepo,
		dailyCheckInRepo:         asDailyCheckInRepository(userRepo),
		dailyCheckInCalendarRepo: asDailyCheckInCalendarRepository(userRepo),
...
func asDailyCheckInRepository(userRepo UserRepository) DailyCheckInRepository {
	repo, _ := userRepo.(DailyCheckInRepository)
	return repo
}

func asDailyCheckInCalendarRepository(userRepo UserRepository) DailyCheckInCalendarRepository {
	repo, _ := userRepo.(DailyCheckInCalendarRepository)
	return repo
}

// GetDailyCheckIn returns the authenticated user's check-in status for today.
func (s *UserService) GetDailyCheckIn(ctx context.Context, userID int64) (DailyCheckIn, error) {
	if s.dailyCheckInRepo == nil {
		return DailyCheckIn{}, fmt.Errorf("daily check-in repository is unavailable")
	}
	result, err := s.dailyCheckInRepo.GetDailyCheckIn(ctx, userID, timezone.Now().Format(time.DateOnly))
	if err != nil {
		return DailyCheckIn{}, fmt.Errorf("get daily check-in: %w", err)
	}
	if !result.ClaimedToday {
		result.Reward = DailyCheckInReward
	}
	return result, nil
}

// ClaimDailyCheckIn credits the daily reward once for the configured calendar day.
func (s *UserService) ClaimDailyCheckIn(ctx context.Context, userID int64) (DailyCheckIn, error) {
	if s.dailyCheckInRepo == nil {
		return DailyCheckIn{}, fmt.Errorf("daily check-in repository is unavailable")
	}

	result, err := s.dailyCheckInRepo.ClaimDailyCheckIn(ctx, userID, timezone.Now().Format(time.DateOnly), DailyCheckInReward)
	if err != nil {
		return DailyCheckIn{}, fmt.Errorf("claim daily check-in: %w", err)
	}
...
// GetDailyCheckInCalendar returns the authenticated user's check-ins for the
// current server month.
func (s *UserService) GetDailyCheckInCalendar(ctx context.Context, userID int64) (DailyCheckInCalendar, error) {
	if s.dailyCheckInCalendarRepo == nil {
		return DailyCheckInCalendar{}, fmt.Errorf("daily check-in calendar repository is unavailable")
	}

	now := timezone.Now()
	monthStart := time.Date(now.Year(), now.Month(), 1, 0, 0, 0, 0, now.Location())
	nextMonth := monthStart.AddDate(0, 1, 0)
	dates, err := s.dailyCheckInCalendarRepo.ListDailyCheckInDates(
		ctx,
		userID,
		monthStart.Format(time.DateOnly),
		nextMonth.Format(time.DateOnly),
	)
	if err != nil {
		return DailyCheckInCalendar{}, fmt.Errorf("get daily check-in calendar: %w", err)
	}

	return DailyCheckInCalendar{
		CheckInDate:  now.Format(time.DateOnly),
		CheckInDates: dates,
	}, nil
}
