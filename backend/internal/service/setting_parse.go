...
func (s *SettingService) InitializeDefaultSettings(ctx context.Context) error {
	...
	defaults := map[string]string{
		...
		// Custom user features (enabled by default; opt-out)
...
func parseIPBlacklistSetting(value string) ([]string, error) {
	...
	// json.Unmarshal accepts `null` for a slice and leaves it nil. Treat it as
	// malformed rather than silently turning a damaged security setting into an
	// empty blacklist.
	if patterns == nil {
		return nil, fmt.Errorf("parse ip_blacklist: value must be a JSON array")
	}
	for _, pattern := range patterns {
		if strings.TrimSpace(pattern) == "" {
			return nil, fmt.Errorf("parse ip_blacklist: entries must not be empty")
		}
	}
	return NormalizeIPBlacklist(patterns)
}
...
func (s *SettingService) parseSettings(settings map[string]string) *SystemSettings {
	...
	ipBlacklist := s.cachedIPBlacklistPatterns()
	if value, ok := settings[SettingKeyIPBlacklist]; ok {
		parsed, err := parseIPBlacklistSetting(value)
		if err != nil {
			slog.Error("invalid persisted ip blacklist; retaining last valid value", "error", err)
		} else {
			ipBlacklist = parsed
		}
	}
