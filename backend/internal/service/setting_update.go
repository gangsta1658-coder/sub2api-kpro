...
// settingsUpdateTouchesIPBlacklist reports whether the write or its cache
// refresh can replace the active site-wide IP blacklist. Partial settings
// writes omit the key from the database update, but refreshCachedSettingsAfterWrite
// still reads and installs the persisted value.
func settingsUpdateTouchesIPBlacklist(updates map[string]string, omitted OmittedSettingKeys) bool {
	if _, writes := updates[SettingKeyIPBlacklist]; writes {
		return true
	}
	_, refreshes := omitted[SettingKeyIPBlacklist]
	return refreshes
}
...
func (s *SettingService) UpdateSettingsOmitting(ctx context.Context, settings *SystemSettings, omitted OmittedSettingKeys) error {
	...
	if settingsUpdateTouchesIPBlacklist(updates, omitted) {
...
func (s *SettingService) buildSystemSettingsUpdates(ctx context.Context, settings *SystemSettings) (map[string]string, error) {
	...
	ipBlacklist, err := NormalizeIPBlacklist(settings.IPBlacklist)
	if err != nil {
		return nil, infraerrors.BadRequest("INVALID_IP_BLACKLIST", err.Error())
	}
	settings.IPBlacklist = ipBlacklist
	ipBlacklistJSON, err := json.Marshal(ipBlacklist)
	if err != nil {
...
func (s *SettingService) refreshCachedSettings(settings *SystemSettings) {
	...
	s.storeIPBlacklist(settings.IPBlacklist)
