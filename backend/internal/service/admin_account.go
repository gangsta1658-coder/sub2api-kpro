...
func (s *adminServiceImpl) BulkUpdateAccounts(ctx context.Context, input *BulkUpdateAccountsInput) (*BulkUpdateAccountsResult, error) {
	...
	if len(input.Credentials) > 0 || input.ProxyID != nil || needMixedChannelCheck || openAISettings.any() || hasLongContextBillingUpdate || input.ProbeEnabled != nil || input.RateMultiplier != nil {
...
func (s *adminServiceImpl) resolveBulkUpdateTargetIDs(ctx context.Context, filters *BulkUpdateAccountFilters) ([]int64, error) {
	...
	folder := strings.TrimSpace(filters.Folder)
	if folder != "" {
		ctx = WithAccountFolderFilter(ctx, folder, folder == AccountFolderUncategorizedFilter)
	}
