...
func (s *UpdateService) getRepo() string {
	if customRepo := strings.TrimSpace(os.Getenv("SUB2API_UPDATE_REPO")); customRepo != "" {
		return customRepo
	}
	if customRepo := strings.TrimSpace(os.Getenv("UPDATE_GITHUB_REPO")); customRepo != "" {
		return customRepo
	}
...
func (s *UpdateService) PerformUpdate(ctx context.Context) error {
	...
	if s.getRepo() == defaultGithubRepo && os.Getenv("SUB2API_ALLOW_OFFICIAL_OVERWRITE") != "true" {
		return infraerrors.Conflict("CUSTOM_FEATURES_PROTECTED", "当前部署包含 Kpro 定制功能（账号文件夹、每日签到、社区留言板、IP黑名单等），直接在线覆盖官方二进制会导致定制功能丢失。请通过无损升级流程进行平滑升级。")
	}
...
func (s *UpdateService) fetchRollbackCandidates(ctx context.Context) ([]*GitHubRelease, error) {
	releases, err := s.githubClient.FetchRecentReleases(ctx, s.getRepo(), rollbackFetchPageSize)
	if err != nil {
		return nil, err
	}
...
func (s *UpdateService) fetchLatestRelease(ctx context.Context) (*UpdateInfo, error) {
	release, err := s.githubClient.FetchLatestRelease(ctx, s.getRepo())
	if err != nil {
		return nil, err
	}
