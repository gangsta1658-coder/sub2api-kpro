  ...
  <AuthLayout>
    <div class="space-y-6">
      ...
      <div
        ...
        class="rounded-xl glass-alert glass-alert-warning"
      >
        <div class="flex items-start gap-3">
          <div class="flex-shrink-0">
            <Icon name="exclamationCircle" size="md" class="text-[#ffd60a]" />
          </div>
          <p class="text-sm text-[#ffd48a]">
      ...
      <form v-else @submit.prevent="handleRegister" class="space-y-5">
        <!-- Email Input -->
        <div>
          <label for="email" class="glass-input-label">
            {{ t('auth.emailLabel') }}
          </label>
          <div class="relative">
            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5">
              <Icon name="mail" size="md" class="text-white/45" />
            </div>
            <input
              id="email"
        ...
        <!-- Password Input -->
        <div>
          <label for="password" class="glass-input-label">
            {{ t('auth.passwordLabel') }}
          </label>
          <div class="relative">
            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5">
              <Icon name="lock" size="md" class="text-white/45" />
            </div>
            <input
              id="password"
            ...
            <button
              ...
              class="absolute inset-y-0 right-0 flex items-center pr-3.5 text-white/45 transition-colors hover:text-white/85"
            >
              <Icon v-if="showPassword" name="eyeOff" size="md" />
              <Icon v-else name="eye" size="md" />
            </button>
          </div>
          <p class="glass-input-hint">
            {{ t('auth.passwordHint') }}
          </p>
        </div>
        ...
        <div v-if="invitationCodeEnabled">
          <label for="invitation_code" class="glass-input-label">
            {{ t('auth.invitationCodeLabel') }}
          </label>
          <div class="relative">
            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5">
              <Icon name="key" size="md" :class="invitationValidation.valid ? 'text-[#34c759]' : 'text-white/45'" />
            </div>
            <input
              id="invitation_code"
            ...
            <div v-if="invitationValidating" class="absolute inset-y-0 right-0 flex items-center pr-3.5">
              <svg class="h-4 w-4 animate-spin text-white/50" fill="none" viewBox="0 0 24 24">
            ...
            <div v-else-if="invitationValidation.valid" class="absolute inset-y-0 right-0 flex items-center pr-3.5">
              <Icon name="checkCircle" size="md" class="text-[#34c759]" />
            </div>
          ...
          <transition name="fade">
            <div v-if="invitationValidation.valid" class="mt-2 flex items-center gap-2 rounded-lg glass-inline-valid">
              <Icon name="checkCircle" size="sm" class="text-[#34c759]" />
              <span class="text-sm text-[#7ee2a0]">
        ...
        <div v-else-if="affiliateEnabled" data-testid="affiliate-invitation-field">
          <label for="affiliate_code" class="glass-input-label">
            {{ t('auth.invitationCodeLabel') }}
            <span class="ml-1 text-xs font-normal text-white/45">({{ t('common.optional') }})</span>
          </label>
          <div class="relative">
            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5">
              <Icon name="key" size="md" class="text-white/45" />
            </div>
            <input
              id="affiliate_code"
        ...
        <div v-if="promoCodeEnabled">
          <label for="promo_code" class="glass-input-label">
            {{ t('auth.promoCodeLabel') }}
            <span class="ml-1 text-xs font-normal text-white/45">({{ t('common.optional') }})</span>
          </label>
          <div class="relative">
            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5">
              <Icon name="gift" size="md" :class="promoValidation.valid ? 'text-[#34c759]' : 'text-white/45'" />
            </div>
            <input
              id="promo_code"
          ...
          <transition name="fade">
            <div v-if="promoValidation.valid" class="mt-2 flex items-center gap-2 rounded-lg glass-inline-valid">
              <Icon name="gift" size="sm" class="text-[#34c759]" />
              <span class="text-sm text-[#7ee2a0]">
    ...
    <template #footer>
      <p class="text-white/60">
        ...
        <router-link
          to="/login"
          class="font-medium text-[#4da3ff] transition-colors hover:text-white"
        >
          {{ t('auth.signIn') }}
        </router-link>
      </p>
...
import { default as AuthLayout } from '@/components/layout/GlassAuthLayout.vue'
