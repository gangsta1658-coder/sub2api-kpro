<template>
  <AuthLayout>
    <div class="space-y-6">
      ...
      <div class="text-center">
        ...
        <p class="mt-2 text-sm text-white/60">
          {{ t('auth.sendCodeDesc') }}
          <span class="font-medium text-white/90">{{ email }}</span>
        </p>
      </div>
      ...
      <form v-else @submit.prevent="handleVerify" class="space-y-5">
        ...
        <div>
          <label for="code" class="glass-input-label text-center">
          ...
          <input
            ...
            placeholder="000000"
          />
          <p class="glass-input-hint text-center">{{ t('auth.verificationCodeHint') }}</p>
        </div>

        <!-- Code Status -->
        <div
          v-if="codeSent"
          class="rounded-xl glass-alert glass-alert-success"
        >
          <div class="flex items-start gap-3">
            <div class="flex-shrink-0">
              <Icon name="checkCircle" size="md" class="text-[#34c759]" />
            </div>
            <p class="text-sm text-[#7ee2a0]">
        ...
        <div class="text-center">
          <button
            ...
            type="button"
            disabled
            class="cursor-not-allowed text-sm text-white/45"
          >
          ...
          <button
            ...
            "
            class="text-sm text-[#4da3ff] transition-colors hover:text-white disabled:cursor-not-allowed disabled:opacity-50"
          >
    ...
    <template #footer>
      <button
        @click="handleBack"
        class="inline-flex items-center gap-2 text-white/60 transition-colors hover:text-white"
      >
