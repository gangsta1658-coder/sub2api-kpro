  ...
  <AppLayout>
    <TablePageLayout>
      <template #filters>
        ...
        <div class="mt-3 flex flex-col gap-2 border-t border-gray-200 pt-3 dark:border-dark-700">
          <div class="flex min-w-0 items-center gap-2">
            <div class="flex min-w-0 flex-1 items-center gap-1 overflow-x-auto pb-1">
              <button
                type="button"
                class="account-folder-tab"
                :class="{ 'account-folder-tab-active': activeFolder === ALL_ACCOUNTS_FOLDER }"
                @click="selectAccountFolder(ALL_ACCOUNTS_FOLDER)"
              >
                <Icon name="grid" size="sm" />
                <span>{{ t('admin.accounts.folders.allAccounts') }}</span>
                <span class="account-folder-count">{{ accountFolderTotals.total }}</span>
              </button>
              <button
                type="button"
                class="account-folder-tab"
                :class="{ 'account-folder-tab-active': activeFolder === UNCATEGORIZED_FOLDER }"
                @click="selectAccountFolder(UNCATEGORIZED_FOLDER)"
              >
                <Icon name="inbox" size="sm" />
                <span>{{ t('admin.accounts.folders.uncategorized') }}</span>
                <span class="account-folder-count">{{ accountFolderTotals.uncategorized }}</span>
              </button>
              <div
                v-for="folder in accountFolders"
                :key="folder.name"
                class="account-folder-tab group pr-1"
                :class="{ 'account-folder-tab-active': activeFolder === folder.name }"
              >
                <button type="button" class="flex min-w-0 items-center gap-1.5" @click="selectAccountFolder(folder.name)">
                  <Icon name="inbox" size="sm" />
                  <span class="max-w-40 truncate">{{ folder.name }}</span>
                  <span class="account-folder-count">{{ folder.count }}</span>
                </button>
                <button
                  type="button"
                  class="account-folder-action"
                  :title="t('admin.accounts.folders.rename')"
                  @click="openFolderEditor('rename', folder.name)"
                >
                  <Icon name="edit" size="xs" />
                </button>
                <button
                  type="button"
                  class="account-folder-action account-folder-action-danger"
                  :title="t('admin.accounts.folders.delete')"
            ...
            <button type="button" class="btn btn-secondary flex-shrink-0 px-3" @click="openFolderEditor('create')">
              <Icon name="plus" size="sm" />
              <span class="hidden sm:inline">{{ t('admin.accounts.folders.create') }}</span>
            </button>
          </div>

          <div v-if="folderEditorMode || selIds.length > 0" class="flex flex-wrap items-center justify-between gap-2">
            <form v-if="folderEditorMode" class="flex min-w-0 items-center gap-2" @submit.prevent="submitFolderEditor">
              <input
                v-model="folderEditorName"
                type="text"
                maxlength="64"
                class="input h-9 w-56 max-w-full"
                :placeholder="t('admin.accounts.folders.namePlaceholder')"
                @keydown.esc.prevent="closeFolderEditor"
              />
              <button type="submit" class="btn btn-primary h-9 px-3" :disabled="folderSaving || !folderEditorName.trim()">
                <Icon name="check" size="sm" />
                <span>{{ t('common.save') }}</span>
              </button>
              <button type="button" class="btn btn-secondary h-9 px-2" :title="t('common.cancel')" @click="closeFolderEditor">
                <Icon name="x" size="sm" />
              </button>
            </form>

            <div v-if="selIds.length > 0" class="ml-auto flex flex-wrap items-center gap-2">
              <span class="text-sm text-gray-600 dark:text-gray-300">
                {{ t('admin.accounts.folders.selectedAccounts', { count: selIds.length }) }}
              </span>
              <select v-model="moveFolderTarget" class="input h-9 min-w-40 py-1.5">
                <option :value="UNCATEGORIZED_FOLDER">{{ t('admin.accounts.folders.uncategorized') }}</option>
                <option v-for="folder in accountFolders" :key="folder.name" :value="folder.name">{{ folder.name }}</option>
              </select>
              <button type="button" class="btn btn-primary h-9 px-3" :disabled="movingAccounts" @click="moveSelectedAccounts">
                <Icon name="swap" size="sm" />
                <span>{{ t('admin.accounts.folders.move') }}</span>
              </button>
            </div>
          </div>
        </div>
    ...
    <ConfirmDialog
      ...
      :title="t('admin.accounts.folders.delete')"
      :message="t('admin.accounts.folders.deleteConfirm', { name: deletingFolder?.name, count: deletingFolder?.count ?? 0 })"
      :confirm-text="t('common.delete')"
      :cancel-text="t('common.cancel')"
      :danger="true"
      @confirm="confirmDeleteFolder"
      @cancel="showDeleteFolderDialog = false"
    />
...
type AccountFolderSummary = { name: string; count: number }
type FolderEditorMode = 'create' | 'rename'
const ALL_ACCOUNTS_FOLDER = '__all__'
const UNCATEGORIZED_FOLDER = '__uncategorized__'
const EMPTY_ACCOUNT_FOLDERS_STORAGE_KEY = 'account-empty-folders-v1'
const persistedAccountFolders = ref<AccountFolderSummary[]>([])
const emptyAccountFolders = ref<string[]>([])
const accountFolderTotals = reactive({ total: 0, uncategorized: 0 })
const activeFolder = ref(ALL_ACCOUNTS_FOLDER)
const folderEditorMode = ref<FolderEditorMode | null>(null)
const folderEditorName = ref('')
const editingFolderName = ref<string | null>(null)
const folderSaving = ref(false)
const movingAccounts = ref(false)
const moveFolderTarget = ref(UNCATEGORIZED_FOLDER)
const showDeleteFolderDialog = ref(false)
const deletingFolder = ref<AccountFolderSummary | null>(null)
const accountFolders = computed<AccountFolderSummary[]>(() => {
  const folders = new Map(persistedAccountFolders.value.map(folder => [folder.name, folder.count]))
  for (const name of emptyAccountFolders.value) {
...
const persistEmptyAccountFolders = () => {
  localStorage.setItem(EMPTY_ACCOUNT_FOLDERS_STORAGE_KEY, JSON.stringify(emptyAccountFolders.value))
}

const loadEmptyAccountFolders = () => {
  try {
    const parsed = JSON.parse(localStorage.getItem(EMPTY_ACCOUNT_FOLDERS_STORAGE_KEY) || '[]')
    if (!Array.isArray(parsed)) return
    const names = parsed
      .filter((value): value is string => typeof value === 'string')
      .map(normalizeFolderName)
      .filter(name => name && name.length <= 64 && name !== UNCATEGORIZED_FOLDER && name !== ALL_ACCOUNTS_FOLDER)
    emptyAccountFolders.value = Array.from(new Set(names))
  } catch {
    emptyAccountFolders.value = []
  }
}

const loadAccountFolders = async () => {
  try {
    const result = await adminAPI.accounts.listFolders()
    persistedAccountFolders.value = result.folders
    accountFolderTotals.total = result.total
    accountFolderTotals.uncategorized = result.uncategorized_count

    const persistedNames = new Set(result.folders.map(folder => folder.name))
    const nextEmptyFolders = emptyAccountFolders.value.filter(name => !persistedNames.has(name))
    if (nextEmptyFolders.length !== emptyAccountFolders.value.length) {
      emptyAccountFolders.value = nextEmptyFolders
      persistEmptyAccountFolders()
    }
  } catch (error) {
    console.error('Failed to load account folders:', error)
    // Folder metadata is auxiliary; a transient failure must not block account actions.
  }
}

const closeFolderEditor = () => {
  folderEditorMode.value = null
  folderEditorName.value = ''
  editingFolderName.value = null
}

const openFolderEditor = (mode: FolderEditorMode, folderName?: string) => {
  folderEditorMode.value = mode
  editingFolderName.value = mode === 'rename' ? folderName ?? null : null
  folderEditorName.value = mode === 'rename' ? folderName ?? '' : ''
}

const selectAccountFolder = async (folder: string) => {
  if (folder === activeFolder.value) return
  activeFolder.value = folder
  moveFolderTarget.value = folder === ALL_ACCOUNTS_FOLDER ? UNCATEGORIZED_FOLDER : folder
  ;(params as any).folder = folder === ALL_ACCOUNTS_FOLDER ? '' : folder
  pagination.page = 1
  clearSelection()
  try {
    await reload()
  } catch (error) {
    console.error('Failed to filter account folder:', error)
  }
}

const renameAccountFolder = async (oldName: string, newName: string) => {
  const persistedFolder = persistedAccountFolders.value.find(folder => folder.name === oldName)
  let updatedCount = 0
  let failedCount = 0
  if (persistedFolder && persistedFolder.count > 0) {
    const result = await adminAPI.accounts.bulkUpdate({
      filters: { folder: oldName },
      extra: { account_folder: newName }
    })
    updatedCount = result.success
    failedCount = result.failed
  }

  emptyAccountFolders.value = emptyAccountFolders.value
    .filter(name => name !== oldName)
    .concat(updatedCount === 0 ? [newName] : [])
  emptyAccountFolders.value = Array.from(new Set(emptyAccountFolders.value))
  persistEmptyAccountFolders()

  if (activeFolder.value === oldName) {
    activeFolder.value = newName
    ;(params as any).folder = newName
    pagination.page = 1
  }
  closeFolderEditor()
  await Promise.all([reload(), loadAccountFolders()])

  if (failedCount > 0) {
    appStore.showError(t('admin.accounts.folders.partialMove', { success: updatedCount, failed: failedCount }))
    return
  }
  appStore.showSuccess(t('admin.accounts.folders.renameSuccess', { name: newName }))
}

const submitFolderEditor = async () => {
  const name = normalizeFolderName(folderEditorName.value)
  if (!name || name.length > 64 || name === ALL_ACCOUNTS_FOLDER || name === UNCATEGORIZED_FOLDER) {
    appStore.showError(t('admin.accounts.folders.invalidName'))
    return
  }
  const duplicate = accountFolders.value.some(folder =>
    folder.name !== editingFolderName.value && folder.name.toLocaleLowerCase() === name.toLocaleLowerCase()
  )
  if (duplicate) {
    appStore.showError(t('admin.accounts.folders.duplicateName'))
    return
  }

  folderSaving.value = true
  try {
    if (folderEditorMode.value === 'rename' && editingFolderName.value) {
      if (editingFolderName.value === name) {
        closeFolderEditor()
        return
      }
      await renameAccountFolder(editingFolderName.value, name)
      return
    }

    emptyAccountFolders.value = [...emptyAccountFolders.value, name]
    persistEmptyAccountFolders()
    closeFolderEditor()
    appStore.showSuccess(t('admin.accounts.folders.createSuccess', { name }))
    await selectAccountFolder(name)
  } catch (error) {
    console.error('Failed to save account folder:', error)
    appStore.showError(extractApiErrorMessage(error, t('admin.accounts.folders.saveFailed')))
  } finally {
...
const moveSelectedAccounts = async () => {
  ...
  if (accountIds.length === 0 || movingAccounts.value) return

  movingAccounts.value = true
  try {
    const targetName = moveFolderTarget.value === UNCATEGORIZED_FOLDER ? '' : moveFolderTarget.value
    const result = await adminAPI.accounts.bulkUpdate(accountIds, {
      extra: { account_folder: targetName }
    })
    if (result.failed > 0) {
      setSelectedIds(result.failed_ids ?? [])
      appStore.showError(t('admin.accounts.folders.partialMove', { success: result.success, failed: result.failed }))
    } else {
      clearSelection()
      appStore.showSuccess(t('admin.accounts.folders.moveSuccess', { count: result.success }))
    }
    await Promise.all([reload(), loadAccountFolders()])
  } catch (error) {
    console.error('Failed to move accounts to folder:', error)
    appStore.showError(extractApiErrorMessage(error, t('admin.accounts.folders.moveFailed')))
  } finally {
    movingAccounts.value = false
  }
}

const requestDeleteFolder = (folder: AccountFolderSummary) => {
...
const confirmDeleteFolder = async () => {
  ...
  folderSaving.value = true
  try {
    let result: Awaited<ReturnType<typeof adminAPI.accounts.bulkUpdate>> | null = null
    if (folder.count > 0) {
      result = await adminAPI.accounts.bulkUpdate({
        filters: { folder: folder.name },
        extra: { account_folder: '' }
      })
    }
    emptyAccountFolders.value = emptyAccountFolders.value.filter(name => name !== folder.name)
    persistEmptyAccountFolders()
    if (activeFolder.value === folder.name) {
      activeFolder.value = UNCATEGORIZED_FOLDER
      moveFolderTarget.value = UNCATEGORIZED_FOLDER
      ;(params as any).folder = UNCATEGORIZED_FOLDER
      pagination.page = 1
    }
    clearSelection()
    await Promise.all([reload(), loadAccountFolders()])
    if (result && result.failed > 0) {
      ...
      appStore.showSuccess(t('admin.accounts.folders.deleteSuccess', { name: folder.name }))
    }
  } catch (error) {
    console.error('Failed to delete account folder:', error)
    appStore.showError(extractApiErrorMessage(error, t('admin.accounts.folders.deleteFailed')))
  } finally {
...
const buildBulkEditFilterSnapshot = () => {
  ...
  return {
    ...
    search: typeof rawParams.search === 'string' ? rawParams.search : '',
    privacy_mode: typeof rawParams.privacy_mode === 'string' ? rawParams.privacy_mode : '',
    folder: typeof rawParams.folder === 'string' ? rawParams.folder : '',
...
const accountMatchesCurrentFilters = (account: Account) => {
  ...
  const accountFolder = typeof account.extra?.account_folder === 'string' ? account.extra.account_folder.trim() : ''
  const folderFilter = String((params as any).folder || '')
  if (folderFilter) {
    if (folderFilter === UNCATEGORIZED_FOLDER) {
      if (accountFolder !== '') return false
    } else if (accountFolder !== folderFilter) {
      return false
    }
  }
...
onMounted(async () => {
  ...
  loadEmptyAccountFolders()
  void loadAccountFolders()
  load()
...
.account-folder-tab {
  @apply inline-flex h-9 flex-shrink-0 items-center gap-1.5 rounded-md border border-transparent px-3 text-sm font-medium text-gray-600 transition-colors hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-dark-700;
}

.account-folder-tab-active {
...
.account-folder-count {
  @apply inline-flex min-w-5 items-center justify-center rounded bg-gray-100 px-1.5 py-0.5 text-xs text-gray-500 dark:bg-dark-700 dark:text-gray-300;
}

.account-folder-tab-active .account-folder-count {
  @apply bg-primary-100 text-primary-700 dark:bg-primary-900/50 dark:text-primary-300;
}

.account-folder-action {
  @apply inline-flex h-7 w-7 items-center justify-center rounded text-gray-400 transition-colors hover:bg-white hover:text-gray-700 dark:hover:bg-dark-800 dark:hover:text-gray-100;
}

.account-folder-action-danger {
