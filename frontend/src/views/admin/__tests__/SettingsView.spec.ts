...
const {
  ...
  getSiteIPBlacklist: vi.fn().mockResolvedValue({ ips: [] }),
  addSiteIPBlacklist: vi.fn().mockResolvedValue({ ips: [] }),
  removeSiteIPBlacklist: vi.fn().mockResolvedValue({ ips: [] }),
...
describe("admin SettingsView payment visible method controls", () => {
  beforeEach(() => {
    ...
    getSiteIPBlacklist.mockReset();
    addSiteIPBlacklist.mockReset();
    removeSiteIPBlacklist.mockReset();
    ...
    getSiteIPBlacklist.mockResolvedValue({ ips: [] });
    addSiteIPBlacklist.mockImplementation(async ({ ip }: { ip: string }) => ({ ips: [ip] }));
    removeSiteIPBlacklist.mockImplementation(async () => ({ ips: [] }));
  ...
  it("validates and adds a site-wide IP blacklist entry", async () => {
    getSiteIPBlacklist.mockResolvedValueOnce({ ips: ["1.2.3.4"] });
    addSiteIPBlacklist.mockResolvedValueOnce({
      ips: ["1.2.3.4", "47.92.239.70"],
    });
    ...
    const card = wrapper.get('[data-testid="site-ip-blacklist"]');
    expect(card.findAll('[data-testid="site-ip-blacklist-entry"]')).toHaveLength(1);

    const input = card.get('[data-testid="site-ip-blacklist-input"]');
    await input.setValue("not-an-ip");
    await card.get('[data-testid="site-ip-blacklist-add"]').trigger("click");
    expect(addSiteIPBlacklist).not.toHaveBeenCalled();
    expect(card.find('[data-testid="site-ip-blacklist-error"]').exists()).toBe(true);

    await input.setValue("47.92.239.70");
    await card.get('[data-testid="site-ip-blacklist-add"]').trigger("click");
    await flushPromises();

    expect(addSiteIPBlacklist).toHaveBeenCalledWith({ ip: "47.92.239.70" });
    expect(card.findAll('[data-testid="site-ip-blacklist-entry"]')).toHaveLength(2);
  });
