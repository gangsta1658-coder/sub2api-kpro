    ...
    <div class="mx-auto max-w-6xl space-y-6">
      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
        ...
        <div v-show="activeTab === 'security'" class="space-y-6">
          ...
          <!-- Site-wide IP blacklist -->
          <div class="card" data-testid="site-ip-blacklist">
            <div
              class="border-b border-gray-100 px-6 py-4 dark:border-dark-700"
            >
              <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
                {{ t("admin.settings.security.ipBlacklistTitle") }}
              </h2>
              <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                {{ t("admin.settings.security.ipBlacklistDescription") }}
              </p>
            </div>
            <div class="space-y-4 p-6">
              <div
                ...
                role="group"
                :aria-label="t('admin.settings.security.ipBlacklistAddLabel')"
              >
                <input
                  v-model="siteIPBlacklistDraft"
                  data-testid="site-ip-blacklist-input"
                  type="text"
                  inputmode="text"
                  autocomplete="off"
                  class="input flex-1 font-mono"
                  :placeholder="t('admin.settings.security.ipBlacklistPlaceholder')"
                  :aria-invalid="siteIPBlacklistError ? 'true' : 'false'"
                  @keydown.enter.prevent="addSiteIPBlacklistEntry"
                />
                <button
                  type="button"
                  data-testid="site-ip-blacklist-add"
                  class="btn btn-primary inline-flex items-center justify-center gap-1 sm:mt-0"
                  :disabled="siteIPBlacklistOperating || !siteIPBlacklistDraft.trim()"
                  @click="addSiteIPBlacklistEntry"
                >
                  <Icon name="plus" size="sm" />
                  {{
                    siteIPBlacklistOperating
                      ? t("common.loading")
                      : t("admin.settings.security.ipBlacklistAdd")
                  }}
                </button>
              </div>
              <p
                v-if="siteIPBlacklistError"
                class="text-sm text-red-600 dark:text-red-400"
                data-testid="site-ip-blacklist-error"
              >
                {{ siteIPBlacklistError }}
              </p>
              <p class="text-xs text-gray-500 dark:text-gray-400">
                {{ t("admin.settings.security.ipBlacklistHint") }}
              </p>

              <div
                v-if="siteIPBlacklistLoading"
                class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400"
                data-testid="site-ip-blacklist-loading"
              >
                <div
                  class="h-4 w-4 animate-spin rounded-full border-b-2 border-primary-600"
                ></div>
              ...
              <div
                v-else-if="siteIPBlacklistEntries.length === 0"
                ...
                data-testid="site-ip-blacklist-empty"
              >
                {{ t("admin.settings.security.ipBlacklistEmpty") }}
              </div>
              <ul v-else class="divide-y divide-gray-100 rounded-lg border border-gray-200 dark:divide-dark-700 dark:border-dark-600">
                <li
                  v-for="ip in siteIPBlacklistEntries"
                  :key="ip"
                  class="flex items-center justify-between gap-3 px-3 py-2.5"
                  data-testid="site-ip-blacklist-entry"
                >
                  <code class="min-w-0 break-all font-mono text-sm text-gray-800 dark:text-gray-100">{{ ip }}</code>
                  <button
                    type="button"
                    class="btn btn-secondary btn-sm inline-flex flex-shrink-0 items-center gap-1 text-red-600 hover:text-red-700 dark:text-red-400"
                    :aria-label="t('admin.settings.security.ipBlacklistRemove', { ip })"
                    :disabled="siteIPBlacklistOperating"
                    @click="removeSiteIPBlacklistEntry(ip)"
                  >
...
// Site-wide IP blacklist (managed independently from per-key IP ACLs).
const siteIPBlacklistLoading = ref(true);
const siteIPBlacklistOperating = ref(false);
const siteIPBlacklistEntries = ref<string[]>([]);
const siteIPBlacklistDraft = ref("");
const siteIPBlacklistError = ref("");
...
async function saveSettings() {
  ...
  try {
    ...
    const payload: UpdateSettingsRequest = {
      ...
      daily_check_in_enabled: form.daily_check_in_enabled,
...
function isValidIPv6(value: string): boolean {
  // IPv4-mapped addresses are valid IPv6 literals and contain a dotted tail.
  const ipv4Tail = value.match(/^(.*:)(\d{1,3}(?:\.\d{1,3}){3})$/);
  if (ipv4Tail) {
    if (!isValidIPv4(ipv4Tail[2])) return false;
    value = `${ipv4Tail[1]}0:0`;
  }
  ...
  const validPiece = (piece: string) => /^[0-9a-fA-F]{1,4}$/.test(piece);
  if (!left.every(validPiece) || !right.every(validPiece)) return false;
  return pieces.length === 2
    ? left.length + right.length < 8
    : left.length === 8;
}

function isValidSiteIPBlacklistEntry(raw: string): boolean {
  const value = raw.trim();
  if (!value || /\s/.test(value)) return false;
  const slash = value.indexOf("/");
  const address = slash >= 0 ? value.slice(0, slash) : value;
  const prefix = slash >= 0 ? value.slice(slash + 1) : "";
  ...
  return prefixNumber >= 0 && prefixNumber <= (isIPv4 ? 32 : 128);
}

function setSiteIPBlacklistEntries(entries: string[]): void {
  siteIPBlacklistEntries.value = Array.from(
    new Set(
      entries
...
async function loadSiteIPBlacklist() {
  siteIPBlacklistLoading.value = true;
  try {
    const response = await adminAPI.settings.getSiteIPBlacklist();
    setSiteIPBlacklistEntries(response.ips);
  } catch (_error: unknown) {
    // Keep an empty list when the optional endpoint is unavailable.
    siteIPBlacklistEntries.value = [];
  } finally {
    siteIPBlacklistLoading.value = false;
  }
}

async function addSiteIPBlacklistEntry() {
  const ip = siteIPBlacklistDraft.value.trim();
  siteIPBlacklistError.value = "";
  if (!isValidSiteIPBlacklistEntry(ip)) {
    siteIPBlacklistError.value = t("admin.settings.security.ipBlacklistInvalid");
    return;
  }
  if (siteIPBlacklistEntries.value.some((entry) => entry.toLowerCase() === ip.toLowerCase())) {
    siteIPBlacklistError.value = t("admin.settings.security.ipBlacklistDuplicate");
    return;
  }

  siteIPBlacklistOperating.value = true;
  try {
    const response = await adminAPI.settings.addSiteIPBlacklist({ ip });
    if (response.ips.length > 0) {
      setSiteIPBlacklistEntries(response.ips);
    } else {
      setSiteIPBlacklistEntries([...siteIPBlacklistEntries.value, ip]);
    }
    siteIPBlacklistDraft.value = "";
    appStore.showSuccess(t("admin.settings.security.ipBlacklistAdded"));
  } catch (error: unknown) {
    appStore.showError(
      extractApiErrorMessage(error, t("admin.settings.security.ipBlacklistSaveFailed")),
    );
  } finally {
    siteIPBlacklistOperating.value = false;
  }
}

async function removeSiteIPBlacklistEntry(ip: string) {
  if (!confirm(t("admin.settings.security.ipBlacklistRemoveConfirm", { ip }))) return;
  siteIPBlacklistOperating.value = true;
  try {
    const response = await adminAPI.settings.removeSiteIPBlacklist(ip);
    if (response.ips.length > 0) {
      setSiteIPBlacklistEntries(response.ips);
    } else {
      setSiteIPBlacklistEntries(siteIPBlacklistEntries.value.filter((entry) => entry !== ip));
    }
    appStore.showSuccess(t("admin.settings.security.ipBlacklistRemoved"));
  } catch (error: unknown) {
    appStore.showError(
      extractApiErrorMessage(error, t("admin.settings.security.ipBlacklistSaveFailed")),
    );
