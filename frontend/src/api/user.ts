...
export async function getDailyCheckIn(): Promise<DailyCheckInStatus> {
...
export async function getDailyCheckInCalendar(): Promise<DailyCheckInCalendar> {
  const { data } = await apiClient.get<DailyCheckInCalendar>('/user/check-in/calendar')
  return data
}
