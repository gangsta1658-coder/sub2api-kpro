...
export interface SystemSettings {
  ...
  /** Site-wide client IP/CIDR blacklist (managed by the dedicated endpoint). */
  ip_blacklist?: string[];
...
export interface UpdateSettingsRequest {
  ...
  api_key_acl_trust_forwarded_ip?: boolean;
  forwarded_client_ip_headers?: string[];
  /** Site-wide client IP/CIDR blacklist; prefer the dedicated CRUD endpoints. */
  ip_blacklist?: string[];
  linuxdo_connect_enabled?: boolean;
  linuxdo_connect_client_id?: string;
...
/** Site-wide IP/CIDR deny-list entry. */
export interface SiteIPBlacklistResponse {
  ips: string[];
}

export interface AddSiteIPBlacklistRequest {
  ip: string;
}

/**
 * Normalize the backend's list response. Older deployments may return a bare
 * array while newer handlers return `{ ips: [...] }`; accepting both keeps the
 * admin UI compatible during rolling upgrades.
 */
function normalizeSiteIPBlacklistResponse(
  value: SiteIPBlacklistResponse | string[] | { items?: string[]; ip?: string },
): SiteIPBlacklistResponse {
  if (Array.isArray(value)) return { ips: value };
  const candidate = value as SiteIPBlacklistResponse & {
    items?: string[];
    ip?: string;
  };
  if (Array.isArray(candidate.ips)) return { ips: candidate.ips };
  if (Array.isArray(candidate.items)) return { ips: candidate.items };
  return candidate.ip ? { ips: [candidate.ip] } : { ips: [] };
}

export async function getSiteIPBlacklist(): Promise<SiteIPBlacklistResponse> {
  const { data } = await apiClient.get<
    SiteIPBlacklistResponse | string[] | { items?: string[]; ip?: string }
  >("/admin/settings/ip-blacklist");
  return normalizeSiteIPBlacklistResponse(data);
}

export async function addSiteIPBlacklist(
  request: AddSiteIPBlacklistRequest,
): Promise<SiteIPBlacklistResponse> {
  const { data } = await apiClient.post<
    SiteIPBlacklistResponse | string[] | { items?: string[]; ip?: string }
  >("/admin/settings/ip-blacklist", request);
  return normalizeSiteIPBlacklistResponse(data);
}

export async function removeSiteIPBlacklist(ip: string): Promise<SiteIPBlacklistResponse> {
  const { data } = await apiClient.delete<
    SiteIPBlacklistResponse | string[] | { items?: string[]; ip?: string }
  >("/admin/settings/ip-blacklist", { data: { ip } });
  return normalizeSiteIPBlacklistResponse(data);
}
