...
export interface AccountFolderSummary {
  name: string
  count: number
}

export interface AccountFoldersResponse {
  folders: AccountFolderSummary[]
  total: number
  uncategorized_count: number
}

export async function listFolders(): Promise<AccountFoldersResponse> {
  const { data } = await apiClient.get<AccountFoldersResponse>('/admin/accounts/folders')
  return data
}
