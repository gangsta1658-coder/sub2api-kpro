...
const getOptionLabel = (option: any): string => {
  if (typeof option === 'object' && option !== null) {
    const custom = option[props.labelKey]
    if (custom !== undefined && custom !== null && String(custom).trim() !== '') {
      return String(custom)
    }
    if (option.label !== undefined && option.label !== null && String(option.label).trim() !== '') {
      return String(option.label)
    }
    if (option.name !== undefined && option.name !== null && String(option.name).trim() !== '') {
      return String(option.name)
    }
    if (option[props.valueKey] !== undefined && option[props.valueKey] !== null && String(option[props.valueKey]).trim() !== '') {
